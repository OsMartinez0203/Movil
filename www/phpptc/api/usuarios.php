    ...
    if (isset($_SESSION['idUsuario'])) {
        switch ($_GET['action']) {
            case 'logout':
                ...
                break;
            case 'readUsu':
                if ($result['dataset'] = $usuario->readUsu()) {
                    $result['status'] = 1;
                } else {
                                ...
                                if ($usuario->setCorreo($_POST['profile_correo'])) {
                                    if ($usuario->setAlias($_POST['profile_alias'])) {
                                        if ($usuario->updateUsuario()) {
                                            $_SESSION['aliasUsuario'] = $_POST['profile_correo'];
                                            $result['status'] = 1;
                                        } else {
                        ...
                        if ($usuario->setCorreo($_POST['create_correo'])) {
                            if ($usuario->setAlias($_POST['create_alias'])) {
                                if ($_POST['create_clave1'] == $_POST['create_clave2']) {
                                    if ($usuario->setcontra($_POST['create_clave1'])) {
                                        if ($usuario->createUsuario()) {
                                            $result['status'] = 1;
                                            $result['message'] = 'Usuario agregado correctamente.';
                                        } else {
                                            $result['exception'] = 'Elija otro usuario.';
                                        }
                                    } else {
                        ...
                        if ($usuario->setNombres($_POST['update_nombres'])) {
                            if ($usuario->setApellidos($_POST['update_apellidos'])) {
                                if ($usuario->setCorreo($_POST['update_correo'])) {
                                    if ($usuario->setcontra($_POST['update_alias'])) {
                                        if ($usuario->updateUsuario()) {
                                            $result['status'] = 1;
                                        } else {
                                            $result['exception'] = 'Operación fallida';
            ...
            case 'login':
                ...
                if ($usuario->setCorreo($_POST['correo'])) {
                    if ($usuario->checkCorreo()) {
                        if ($usuario->setContra($_POST['contra'])) {
                            if ($usuario->checkContra()) {
                                $_SESSION['idUsuario'] = $usuario->getId();
                                $_SESSION['nombre'] = $usuario->getNombres();
                                $_SESSION['aliasUsuario'] = $usuario->getCorreo();
                                $result['status'] = 1;
                            } else {
                                $result['exception'] = 'Contraseña Incorrecta';
                            }
                        } else {
                            $result['exception'] = 'Ingrese una contraseña mayor a 6 caracteres';
                        }
                    } else {
                        $result['exception'] = 'Tu correo electrónico es incorrecto o inexistente';
                    }
                } else {
                    $result['exception'] = 'Tu correo electrónico es incorrecto o inexistente';
                }
